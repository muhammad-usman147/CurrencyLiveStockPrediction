...
def GetData():
    ...
    clf = joblib.load(path+'gbpusd.pkl')
    ...
    clf = joblib.load(path+'xauusd.pkl')
...
def UploadData():
    ...
    if request.method == "POST":
        ...
        if files:
            files.save(path+f"{file_name}.csv")
...
def admin_login():
    path = 'C:/Users/Modi/Desktop/AllCodingCodes/CurrencyLiveStockPrediction/'
    admin_data = pd.read_csv(path+'admin.csv')
...
def chage_password():
    ...
    admin_data.to_csv(path+'admin.csv',index = False)
    return render_template('admin_login.html')
